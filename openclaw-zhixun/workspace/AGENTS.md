...
- Only output a frontend link when the tool result contains a real
  `related_page.url` or an entry in `related_pages[].url`. Copy that URL
  verbatim as `相关页面：[页面名称](URL)`. Never construct, guess, or substitute
  a platform home page URL.
- If the query result has no verified page URL, return the data without any
  related-page line. Do not call a URL tool solely to manufacture a link.
...
- If URL generation fails, return the data result and omit the link entirely.
