...
COMPAT_VERSION = "2026-08-05-aligned-native-image-v4"
...
def _align_run_reference_time(value: Any) -> Any:
    if not value:
        return value
    try:
        ...
        return value
    minutes = current.hour * 60 + current.minute - 120
    snapped = math.ceil(minutes / 180) * 180 + 120
    if snapped >= 1440:
        current = current + timedelta(days=1)
        snapped %= 1440
    aligned = current.replace(
        hour=snapped // 60, minute=snapped % 60, second=0, microsecond=0
    )
    return aligned.strftime("%Y-%m-%d %H:%M")


def _validate_run_reference_time(value: Any, requested: Any, effective: Any) -> Any:
    if not effective or not isinstance(value, dict):
        return value
    ...
    if not isinstance(returned, str):
        return value
    try:
        effective_dt = datetime.fromisoformat(str(effective).strip().replace("T", " "))
        returned_dt = datetime.fromisoformat(returned.strip().replace("T", " "))
    except ValueError:
        return value
    try:
        delta_hours = (returned_dt - effective_dt).total_seconds() / 3600
    except TypeError:
        return value
    validation = {
        "requested_reference_time": str(requested),
        "effective_reference_time": str(effective),
    ...
    if not validation["valid"]:
        raise RuntimeError(
            "实时预报起报时间不一致: "
            f"请求 {requested}（对齐后 {effective}），后端返回 {returned}，"
            f"相差 {delta_hours:g} 小时"
        )
    return value
    ...
    for name in names:
        ...
        async def wrapped(
            ...
            **kwargs: Any,
            ...
            requested_reference = bound.arguments.get("reference_time")
            effective_reference = requested_reference
            if __name in run_names and requested_reference:
                effective_reference = _align_run_reference_time(requested_reference)
                bound.arguments["reference_time"] = effective_reference
            ...
            if __name in run_names and run_all:
                ...
                result = await __function(**bound.arguments)
            if __name in run_names:
                result = _validate_run_reference_time(
                    result, requested_reference, effective_reference
                )
