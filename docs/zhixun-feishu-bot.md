...
所有实时径流和降雨时序工具都会附加 `time_series_response_contract`，其中按数组
路径给出 `point_count`。机器人必须在本轮重新查询，并按时间顺序输出每一个返回点
的完整时间、数值、单位、来源及模型；范围、累计值、极值和洪峰只能作为逐点表格
之后的补充，不能替代表格，也不能使用省略号或“与之前一致”。实时径流模型固定
返回 16 个三小时时次，即未来 48 小时。如果用户请求未来 3 天，机器人应完整列出
已有 48 小时，并明确指出剩余 24 小时后端未返回数据，不能用旧预报补齐。
...
13. `查询碧流河水库过去三天观测径流和未来三天预报径流。本轮重新查询，不得复用此前回答；按时间顺序列出每一个实测和预报点的完整时间、数值、单位与模型，不得省略。若后端预报不足72小时，列完全部已有点后明确说明缺失时段。`
